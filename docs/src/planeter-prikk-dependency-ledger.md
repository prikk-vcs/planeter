...
| Version | v1.3 (2026-09-23 — adds PK-27/28/29: measured keyless serve/accept behaviour on 0.46.0; v1.2: 2026-09-22 — folds prikk 0.44.0 (GHSA-px5q-233r-6hq5), 0.45.0 (format 7 signature union + key-id fix), and 0.46.0 (`tree`/`cat`/`diff` read verbs)) |
...
| PK-27 | **Keyless serve primitives, measured (0.46.0):** `bundle export --ref`, `sync have`, `sync summary` run with **no key directory at all** (the sandbox condition). **`sync build` does NOT — it refuses with *"maintainer signing is required"*.** A keyless forge can therefore serve an *existing* sealed branch as a bundle and advertise `have`/`summary`, but **cannot build incremental-fetch `PEXCH002` artifacts**. | **CONFIRMED** (measured 2026-09-23) | Corrects RFC 004 D-2's assumption ("fetch needs no key on either side"). Incremental fetch from planeter is **gated** on prikk (a keyless `sync build`, or RFC 155's export) — raised as an ask. Also: a planeter-created repo's `heads/main` cannot exist keylessly (accept yields pending claims only) until RFC 154 adoption (0.48.0+), so bundle-clone from planeter serves only branches that already exist sealed. |
| PK-28 | **Keyless `sync accept` under adversarial input, measured (0.46.0):** a bare `init` repo with no keys/`HOME` accepts a `PEXCH002` → verified pending claims; **re-accepting the same artifact is idempotent** (pending count unchanged); a **byte-flipped artifact is refused** ("declared patch-set digest does not match … refusing before any signature work") with the repo still `verify`-clean; **random bytes are refused** ("invalid patch-exchange artifact magic"). | **CONFIRMED** (measured 2026-09-23) | The push path RFC 004 D-3/D-4/D-7/D-8 needs. planeter still bounds size/count/ratio *before* the confined parse (`cat --max-bytes` lesson, PK-24) and serializes accepts per repo. |
| PK-29 | A **received ref (`remotes/…`) on a keyless receiver** is readable (`tree`/`log`/`show`/`bundle preview`) but **cannot be materialized or re-exported**; a keyed receiver takes it into a local branch with **`prikk merge --from`**. | **CONFIRMED** (measured 2026-09-23) | The client-helper clone path: `bundle import --input` then `merge --from` (client keys). Refines PK-4. |
